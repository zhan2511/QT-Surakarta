...
}; // 定义名为 SurakartaMove 的结构体来记录每次移动棋子的情况，内容具体包括棋子前后位置、移动过程以及玩家信息
...
struct SurakartaGameInfo {
    ...
    bool IsEnd() const { return IsEndReason(end_reason_); } // 判断棋局是否已经结束
    SurakartaPlayer Winner() const { return winner_; } // 找到胜利者
    ...
    friend std::istream& operator>>(std::istream& is, SurakartaGameInfo& game_info) {
        ...
        return is;
    } // 定义友元函数 operator>> 读入以字符形式输入的各项信息
};
